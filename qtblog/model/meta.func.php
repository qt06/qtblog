...
function meta_get_meta_by_slug($slug) {
	$metas = meta_find(array('slug' => $slug), array(), 1, 1);
$meta = empty($metas) ? array() : array_pop($metas);
return $meta;
}
function meta_get_meta_by_name($name) {
	$metas = meta_find(array('name' => $name), array(), 1, 1);
$meta = empty($metas) ? array() : array_pop($metas);
return $meta;
}

function meta_get_category($cate) {
$category = meta_get_meta_by_slug($cate);
return (!empty($category) && $category['type'] == 'category') ? $category : array();
}

function meta_get_tag($tag) {
$tag = meta_get_meta_by_slug($tag);
